...
5. 점수는 `stats/global`에 누적되어 개인 평균으로 계산되고, 상시 공개된다 (아래 "참가 신청자 hover 열람" 참고)
...
### 점수 열람 (마이페이지)
- 마이페이지에 "내 평균 점수" 형태로 본인의 누적 평균(매너/실력/재만남)과 `ratingCount`를 노출 (세부 UI는 구현 시 기존 마이페이지 패턴을 따름)

### 참가 신청자 hover 열람 (신규)
- 게임 일정 게시글의 참가 신청자 목록(`attendees` 표시 영역)에서, 각 참석자 이름에 마우스를 올리면 그 사람의 누적 평균 점수(매너/실력/재만남)와 `ratingCount`를 보여주는 hover 카드가 뜬다
- 열람 조건 없음 — 로그인한 사용자라면 누구나, 본인의 평가 제출 여부와 무관하게 상시 열람 가능 (blind exchange 같은 제약 없음, 데이터는 항상 `stats/global` 누적치를 그대로 노출)
- 표시 항목: 매너/실력/재만남 각 평균(소수 첫째자리) + "N회 평가받음" (`ratingCount`가 0이면 "아직 평가 없음" 표시)
- 구현 참고: 기존에 네이티브 title 툴팁을 스타일드 hover 카드로 바꾼 이력(커밋 `2cb543a`)이 있으므로 그 패턴을 재사용한다
