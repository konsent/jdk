...
- Produces:
  ...
  - `checkKongzTempTrophies(memberStats)`: `string[]` — `computeKongzTempServer` 결과의 `temp`가 60 이상이면 `kongz-hot`, 80 이상이면 `so-hot`도 추가 (둘 다 만족하면 둘 다 반환)
...
test("checkKongzTempTrophies: 60도 이상 80도 미만이면 kongz-hot만", () => {
  const stats = { ratingCount: 1, ratingSum: { manner: 4, skill: 4, again: 4 } };
  assert.deepStrictEqual(checkKongzTempTrophies(stats), ["kongz-hot"]);
});

test("checkKongzTempTrophies: 80도 이상이면 kongz-hot과 so-hot 모두", () => {
  const stats = { ratingCount: 1, ratingSum: { manner: 5, skill: 5, again: 5 } };
...
```js
test("buildTrophyCandidates: 출석100+게시글30+평점10회4.5+온도80 모두 만족하면 전체 트로피 반환", () => {
  const memberStats = {
    attendCount: 100,
    postCount: 30,
    ratingCount: 10,
    ratingSum: { manner: 45, skill: 45, again: 45 }
  };
